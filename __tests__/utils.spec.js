...
describe("Utils spec", () => {
	describe("factoryValidationObj", () => {
		it("Needs 3 parameters  to work", () => {
			expect(() => {
				Utils.factoryValidationObj();
			}).toThrowError("type and key are required");
			expect(() => {
				Utils.factoryValidationObj(null, null, null);
			}).toThrowError("type and key are required");
			expect(() => {
				Utils.factoryValidationObj(false, "required-something");
			}).toThrowError("type and key are required");
			expect(() => {
				Utils.factoryValidationObj(false, "required-something", "key");
			}).not.toThrowError("type and key are required");
		});

		it("Return the current validation obj", () => {
			const obj = Utils.factoryValidationObj(
				false,
				"name-required",
				"name",
			);
	...
	describe("Rules Creator", () => {
		...
		expect(rules.FirstNameRequired({ firstName: "" })).toEqual({
			valid: false,
			error: {
				type: "required",
				key: "firstName",
		...
		expect(rules.LastNameRequired({ lastName: "" })).toEqual({
			valid: false,
			error: {
				type: "required",
				key: "lastName",
		...
		expect(rules.FirstNameCustomLength({})).toEqual({
			valid: false,
			error: {
				type: "customLength",
				key: "firstName",
				message: "firstName should be greater than 1",
			},
		});
		expect(rules.FirstNameRequired({ firstName: "algo" })).toEqual({
			valid: true,
			error: undefined,
		});
		expect(rules.LastNameRequired({ lastName: "algo" })).toEqual({
			valid: true,
			error: undefined,
		});
		expect(
			rules.FirstNameCustomLength({
				firstName: "algo",
				lastName: "algo",
			}),
