 ...
 * @property {{[key: string]: IError[]}} errors - An array of error objects if any validation fails.
 */
 ...
 * @returns {{[key: string]: Rule}} An object where keys are function names and values are validation functions.
 */
 ...
 * @param {{[key: string]: Rule}} rules - Array of validation rules to apply.
 * @param {Object} state - The current state containing the field values.
...
export function validate(
	...
	return Object.keys(rules).reduce((results, key) => {
		...
		if (appliedRule.error) {
			results.errors[appliedRule.error.key] = results.errors[
				appliedRule.error.key
			]
				? results.errors[appliedRule.error.key].concat([
