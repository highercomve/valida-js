...
function ruleCreator(validators, description) {
  ...
  if (description.validate) {
    return function (state) {
      var message = description.message ? description.message(state, type, key) : (0, _messages.defaultMessage)(state, type, key);
      return factoryValidationObj(description.validate(state), type, key, message);
    };
  }

  var validate = validators[description.type] ? validators[description.type](description.stateMap, type, key, description.compareWith, description.defaultValue, description.message) : validators.required(description.stateMap, type, key, description.compareWith, description.defaultValue, description.message);
  return validate;
}
/**
...
function validate() {
  ...
  return Object.keys(rules).reduce(function (results, key) {
    ...
    if (appliedRule.error) {
      results.errors[appliedRule.error.key] = results.errors[appliedRule.error.key] ? results.errors[appliedRule.error.key].concat([buildError(appliedRule.error)]) : [buildError(appliedRule.error)];
    }
